...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    ...
    if (details.url.includes('aisandbox-pa.googleapis.com') && (isImageGen || isVideoGen)) {
      ...
      if (details.requestBody && details.requestBody.raw) {
        try {
          ...
          if (!clientContext.recaptchaToken && body.requests && body.requests[0]) {
            clientContext = body.requests[0].clientContext || clientContext;
          }
          
          const recaptcha = clientContext.recaptchaToken;
...
function injectGrecaptchaHelper(siteKey) {// // 
  ...
  async function generateToken(actionName) {
    if (!isReady()) {
      throw new Error('grecaptcha not ready. Click Generate on labs.google/fx first!');
    }
  ...
  window.addEventListener('message', async (event) => {
    ...
    try {
      ...
      if (requestType === 'generateToken') {
        ...
        const apiResult = await makeApiCall(data.url, data.payload, data.bearerToken);
        result = apiResult;
      }
      else {
...
chrome.runtime.onMessageExternal.addListener((request, sender, sendResponse) => {// 
  ...
  if (request.action === 'ping') {// 
    sendResponse({ success: true, version: '5.23', extensionName: 'VEO Batch Processor' });
    return true;
  }
  ...
  if (request.action === 'getBearerToken') {// 
    chrome.storage.local.get(['latestTokens'], (result) => {
      ...
      if (tokens.bearer) {// 
        ...
        sendResponse({ 
          success: false, 
          error: 'No Bearer token captured. Open labs.google/fx and generate something first.' 
        });
      }
    });
    return true;
  ...
  if (request.action === 'getAllTokens') {// 
    ...
    chrome.storage.local.get(['latestTokens'], (result) => {
      ...
      if (!tokens.bearer) {
        sendResponse({ 
          success: false, 
          error: 'No Bearer token. Open labs.google/fx and generate something first.' 
        });
        return;
      }
      ...
      chrome.tabs.query({ url: 'https://labs.google/*' }, (tabs) => {
        ...
        chrome.tabs.sendMessage(labsTab.id, {
          ...
          if (chrome.runtime.lastError || !response?.success) {
            ...
            sendResponse({ 
              ...
              bearer: tokens.bearer,
              recaptcha: null,
              warning: 'Failed to get fresh reCAPTCHA: ' + (chrome.runtime.lastError?.message || response?.error)
            });
            return;
          }
