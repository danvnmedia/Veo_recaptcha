// Content script for labs.google v5.24
// BATCH PROCESSING + AUTO GENERATION MODE
// FIXED: Proper queue sync with auto_generator
// DEBUG: Added logging for connection issues

console.log(
  "🔌 [labs_content.js] Content script LOADED on:",
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  console.log("🔌 [labs_content.js] Received message:", request.action);
  ...
  if (request.action === "generateRecaptchaToken") {
    console.log("🔌 [labs_content.js] generateRecaptchaToken requested");
    let responded = false;
      ...
      if (event.data?.type === "veo_recaptchaToken") {
        ...
        if (!responded) {
          ...
          if (event.data.success && event.data.token) {
            console.log("🔌 [labs_content.js] Token generated successfully");
    ...
    // Timeout after 10 seconds - MUST call sendResponse!
    setTimeout(() => {
      window.removeEventListener("message", handler);
      if (!responded) {
        responded = true;
        console.log("🔌 [labs_content.js] Token generation TIMEOUT!");
        sendResponse({
          success: false,
          error: "Token generation timeout. Reload labs.google/fx",
        });
      }
    }, 10000);
    return true;
  ...
  if (request.action === "proxyApiCall") {
    ...
    console.log("🔌 [labs_content.js] proxyApiCall requested, id:", requestId);
    let responded = false;

    const handler = (event) => {
      // Only handle response that matches OUR request ID
      if (
        event.data?.type === "veo_apiCallResult" &&
        ...
        if (!responded) {
          ...
          if (event.data.success) {
            console.log("🔌 [labs_content.js] API call success");
            sendResponse({
              ok: event.data.ok,
    ...
    // Timeout after 60 seconds - MUST call sendResponse!
    setTimeout(() => {
      window.removeEventListener("message", handler);
      if (!responded) {
        responded = true;
        console.log("🔌 [labs_content.js] proxyApiCall TIMEOUT!");
        sendResponse({ success: false, error: "API call timeout (60s)" });
      }
    }, 60000);
    return true;
  }
  ...
  if (request.action === "queueVideoRequest") {
    ...
    updateOverlay();
    showNotification(
      "✅ Prompt added to queue! Click Generate or enable Auto Mode.",
    );
...
window.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "flowHarvester_videoResponse") {
    ...
    if (processingIndex >= 0 && promptQueue[processingIndex]) {
      promptQueue[processingIndex].status = event.data.success
        ? "completed"
        : "failed";
    }
    ...
    if (!isAutoMode && event.data.success) {
      ...
      updateOverlay();
      showNotification(
        `✅ Video ${currentIndex}/${promptQueue.length} queued!`,
      );
    }
  }
  ...
  if (event.data?.type === "veoAutoGen_stopped") {
    ...
    updateOverlay();
    showNotification(
      `⏹️ Stopped at ${event.data.processed}/${event.data.total}`,
    );
  }
});
...
function updateOverlay() {
  ...
  const percent =
    promptQueue.length > 0
      ? Math.round((currentIndex / promptQueue.length) * 100)
      : 0;
